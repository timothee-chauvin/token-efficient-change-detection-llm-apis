...
def prompt_rows_dict_to_completion_sample(
    prompt_rows_dict: dict[str, list[CompressedOutputRow]],
    tokenizer: PreTrainedTokenizerBase,
    max_tokens: int | None = None,
) -> CompletionSample:
    if max_tokens is None:
        max_tokens = config.sampling.gao2025.max_tokens
    prompts_list = []
    completions_list = []
...
def gao2025_two_sample_test(
    ...
    sample1 = prompt_rows_dict_to_completion_sample(rows_subset, tokenizer, max_tokens)
    sample2 = prompt_rows_dict_to_completion_sample(unchanged_rows_subset, tokenizer, max_tokens)
