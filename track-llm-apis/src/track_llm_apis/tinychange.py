...
class TinyChangeConfig(BaseSettings):
    ...
    random_noise_scale: list[float | int] = Field(
        default_factory=lambda: [float(2 ** (-n)) for n in range(0, 31)]
    )
    weight_pruning_magnitude_scale: list[float | int] = Field(
        default_factory=lambda: [float(2 ** (-n)) for n in range(0, 21)]
    )
    ...
    finetuning_samples: list[int] = Field(default_factory=lambda: [2**n for n in range(0, 13)])
