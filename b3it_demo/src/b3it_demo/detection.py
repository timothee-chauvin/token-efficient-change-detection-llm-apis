...
Vendored unchanged from the production monitor (bi/detection.py); only the
import paths differ.
"""
...
from b3it_demo.analyze import Timestamp, compute_tv_distance, get_distribution
from b3it_demo.config import config


def epoch_tv_series(reference: dict[str, list], results: dict) -> list[tuple[Timestamp, float]]:
