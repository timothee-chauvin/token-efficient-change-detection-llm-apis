...
from b3it_monitoring.bi.phase_2 import (
    Prompt,
    ResponseToken,
    Timestamp,
    load_border_inputs,
...
from b3it_monitoring.util import endpoint_from_slug, slugify
...
def plot_tv_distance_over_time(
    ...
    for endpoint_dir in endpoint_dirs:
        ...
        if max(tv_values) < min_tv_threshold:
            continue
...
def has_transition_pattern(
    tv_over_time: list[tuple[Timestamp, float]],
    threshold: float,
    days_before: int,
    days_after: int,
) -> bool:
    """Check if TV series has consecutive days below threshold followed by consecutive days above."""
    ...
    for i in range(len(tv_values) - days_before - days_after + 1):
        before_ok = all(tv < threshold for tv in tv_values[i : i + days_before])
        after_ok = all(
            tv >= threshold
            for tv in tv_values[i + days_before : i + days_before + days_after]
        )
...
def plot_tv_distance_transitions(
    ...
    for endpoint_dir in endpoint_dirs:
        ...
        if not has_transition_pattern(tv_over_time, threshold, days_before, days_after):
            continue
    ...
    fig.update_layout(
        ...
        width=800,
        yaxis=dict(range=[0, 1]),
        legend=dict(yanchor="top", y=0.99, xanchor="left", x=0),
